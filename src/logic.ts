...
export class Game {
  ...
  constructor() {
    ...
    let dom = document.querySelector('.ui') as HTMLDivElement
  ...
  _handleMouseUp() {
    ...
    if (this.jumper.position.y >= 1) {
      ...
      this.jumperStat.ySpeed -= 0.02 //上升落下状态
      this._render()
      requestAnimationFrame(() => {
        //循环执行
        this._handleMouseUp()
  ...
  _setRenderer() {
    ...
    let dom: any = document.querySelector('.game-ui')
    dom.appendChild(this.renderer.domElement) //渲染的画布放到body里面
  }
  //设置灯光
  _setLight() {
  ...
  _updateCamera() {
    ...
    if (cur.z > next.z) {
      ...
      if (this.cameraPros.current.z - this.cameraPros.next.z < 0.05) {
